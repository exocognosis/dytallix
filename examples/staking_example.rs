...
fn example_basic_staking() {
    ...
    staking
        .delegate(validator_addr.clone(), validator_addr.clone(), self_stake)
    ...
    staking
        .process_block_rewards(1)
        .expect("Failed to process rewards");
...
fn example_multiple_participants() {
    ...
    for (val_addr, self_stake, commission) in validators {
        staking
            .register_validator(val_addr.to_string(), vec![0x01, 0x02], commission)
            .expect("Failed to register validator");

        staking
            .delegate(val_addr.to_string(), val_addr.to_string(), self_stake)
            .expect("Failed to self-delegate");
    ...
    for validator in active_validators {
        println!(
            "      {} - {} uDGT total stake",
            validator.address, validator.total_stake
        );
    }
    println!();
}
...
fn example_reward_distribution() {
    ...
    staking
        .register_validator("validator".to_string(), vec![0x01], 0)
    ...
    for block in 1..=5 {
        staking
            .process_block_rewards(block)
            .expect("Failed to process block");

        let rewards_a = staking
            .calculate_pending_rewards(&"delegator_a".to_string(), &"validator".to_string())
            .expect("Failed to calculate A rewards");
        let rewards_b = staking
            .calculate_pending_rewards(&"delegator_b".to_string(), &"validator".to_string())
            .expect("Failed to calculate B rewards");
        let rewards_val = staking
            .calculate_pending_rewards(&"validator".to_string(), &"validator".to_string())
