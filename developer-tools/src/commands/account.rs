...
pub async fn create_account(name: Option<String>, config: &Config) -> Result<()> {
    ...
    if let Some(account_info) = crypto_manager.get_account_info(&account_name)? {
        ...
        println!("  Key Exchange Public Key: {}...", &account_info.key_exchange_public_key_hex[..32]);
    }
...
pub async fn list_accounts(config: &Config) -> Result<()> {
    ...
    if accounts.is_empty() {
        println!("{}", "No accounts found. Create one with 'dytallix-cli account create'".bright_yellow());
        return Ok(());
    }
...
pub async fn account_balance(account: String, config: &Config) -> Result<()> {
    ...
    if let Some(account_info) = crypto_manager.get_account_info(&account)? {
        ...
        println!("  Key Exchange Algorithm: {}", account_info.key_exchange_algorithm);
        println!("  Created: {}", format_timestamp(account_info.created_at));
...
pub async fn export_account(account: String, output: Option<String>, config: &Config) -> Result<()> {
    ...
    if include_private_keys {
        println!("{}", "🔒 Remember to keep your private key export secure!".bright_yellow());
    }
...
pub async fn sign_message(account: String, message: String, config: &Config) -> Result<()> {
    ...
    let signature = crypto_manager.sign_message(
        &account,
        message.as_bytes(),
        passphrase.as_deref(),
    )?;
...
pub async fn verify_signature(
    ...
    let is_valid = crypto_manager.verify_signature(
        message.as_bytes(),
        &signature,
        &public_key,
        sig_alg,
...
pub async fn generate_address(config: &Config) -> Result<()> {
    ...
    println!("Key Exchange Public Key: {}", kex_public_key.bright_purple());
