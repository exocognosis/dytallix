    ...
    let from_address = match from {
        ...
        None => {
            ...
            if accounts.is_empty() {
                println!(
                    "{}",
                    "❌ No accounts found. Please create an account first.".bright_red()
                );
        ...
        Ok(response) => {
            if response.success {
                if let Some(balance) = response.data {
                    println!(
                        "Current balance: {}",
                        format_amount_with_symbol(balance, "udgt").bright_green()
                    );
        ...
        Ok(response) => {
            if response.success {
                if let Some(tx_data) = response.data {
                    println!(
                        "{}",
                        "✅ Transaction submitted successfully!".bright_green()
                    );
                    ...
                    println!(
                        "{}",
                        "✅ Transaction submitted but no details returned".bright_green()
                    );
                }
            } else {
                println!(
        ...
        Ok(response) => {
            if response.success {
                if let Some(tx_data) = response.data {
                    ...
                    println!(
                        "  Amount: {}",
                        format_amount_with_symbol(tx_data.amount, "udgt").bright_white()
                    );
                    println!(
                        "  Fee: {}",
                        format_amount_with_symbol(tx_data.fee, "udgt").bright_blue()
                    );
        ...
        Ok(response) => {
            if response.success {
                if let Some(transactions) = response.data {
                    println!(
                        "{}",
                        format!("✅ Found {} transactions", transactions.len()).bright_green()
                    );
                    ...
                    for (i, tx) in transactions.iter().enumerate() {
                        ...
                        println!(
                            "  From: {} → To: {}",
                            tx.from.bright_yellow(),
                            tx.to.bright_cyan()
                        );
                        println!(
                            "  Amount: {}",
                            format_amount_with_symbol(tx.amount, "udgt").bright_white()
                        );
