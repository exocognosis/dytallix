...
use dytallix_contracts::tokenomics::{
    DGTToken, DRTToken, EmissionController, TokenomicsProposal, EmissionParameters
};
...
impl TokenomicsTestHarness {
    ...
    pub fn new() -> Self {
        ...
        dgt_token.mint_initial_supply("dyt1treasury".to_string(), 1_000_000).unwrap();
        drt_token.set_emission_controller("dyt1emission_controller".to_string()).unwrap();
        
        emission_controller.set_drt_token("dyt1drt_token".to_string()).unwrap();
        emission_controller.set_governance_contract("dyt1governance".to_string()).unwrap();
        emission_controller.set_treasury("dyt1treasury".to_string()).unwrap();
    ...
    pub fn setup_test_accounts(&mut self) -> HashMap<String, u128> {
        ...
        for (user, amount) in test_users {
            self.dgt_token.transfer(
                "dyt1treasury".to_string(),
                user.to_string(),
                amount
            ).unwrap();
...
mod tokenomics_integration_tests {
    ...
    fn test_adaptive_emission_scenarios() {
        ...
        for (utilization, description) in scenarios {
            ...
            let rate = harness.emission_controller.calculate_adaptive_rate(utilization);
    ...
    fn test_drt_burn_mechanics() {
        ...
        harness.drt_token
            .mint("dyt1user".to_string(), 1000, &"dyt1emission_controller".to_string())
            .unwrap();
        ...
        // Verify burn effects
        assert_eq!(harness.drt_token.total_supply(), initial_supply - burn_amount);
        assert_eq!(harness.drt_token.total_burned(), initial_burned + burn_amount);
        assert_eq!(harness.drt_token.balance_of(&"dyt1user".to_string()), 1000 - burn_amount);
    }
    ...
    fn test_complex_emission_parameter_update() {
        ...
        assert_eq!(updated_params.base_emission_rate, new_params.base_emission_rate);
        assert_eq!(updated_params.max_emission_rate, new_params.max_emission_rate);
        assert_eq!(updated_params.min_emission_rate, new_params.min_emission_rate);
        assert_eq!(updated_params.adjustment_factor, new_params.adjustment_factor);
    }
    ...
    fn test_emission_bounds_enforcement() {
        ...
        for (utilization, description) in extreme_scenarios {
            ...
            assert!(rate >= params.min_emission_rate, "Rate below minimum for {}", description);
            assert!(rate <= params.max_emission_rate, "Rate above maximum for {}", description);
        }
    }
    ...
    fn test_multi_user_governance_scenario() {
        ...
        assert_eq!(harness.dgt_token.balance_of(&"dyt1alice".to_string()), 10000);
        assert_eq!(harness.dgt_token.balance_of(&"dyt1bob".to_string()), 15000);
        assert_eq!(harness.dgt_token.balance_of(&"dyt1charlie".to_string()), 8000);
...
mod tokenomics_performance_tests {
    ...
    fn test_emission_calculation_performance() {
        ...
        for utilization in (0..=10000).step_by(100) {
            let _rate = harness.emission_controller.calculate_adaptive_rate(utilization);
        }
...
mod tokenomics_error_tests {
    ...
    fn test_double_initial_mint_prevention() {
        ...
        let result = harness.dgt_token.mint_initial_supply("dyt1attacker".to_string(), 1_000_000);
    ...
    fn test_unauthorized_proposal_approval() {
        ...
        let result = harness.emission_controller
