...
impl Mempool {
    ...
    fn validate_tx_funds_only(
        ...
        // Skip validation for gasless anchoring (zero-fee Data-only transactions)
        let is_data_only = tx.messages.as_ref().map_or(false, |msgs| {
            msgs.iter().all(|m| matches!(m, crate::storage::tx::TxMessage::Data { .. }))
        });
        let is_gasless = tx.fee == 0 && is_data_only;
        
        if is_gasless {
            eprintln!("[INFO] [Mempool] Gasless anchoring transaction detected - skipping balance checks");
            return Ok(());
        }
