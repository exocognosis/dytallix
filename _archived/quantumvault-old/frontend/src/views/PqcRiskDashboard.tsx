...
import { useRiskSummary, useRiskAssets } from '../api/riskApi';

const PqcRiskDashboard: React.FC = () => {
  const navigate = useNavigate();
  const { data: summary, loading: summaryLoading, error: summaryError } = useRiskSummary();
  const [riskClassFilter, setRiskClassFilter] = useState<string>('');
  const [envFilter, setEnvFilter] = useState<string>('');
  const [searchTerm, setSearchTerm] = useState<string>('');
  
  const { data: assets, loading: assetsLoading, error: assetsError } = useRiskAssets({
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              {['Critical', 'High', 'Medium', 'Low'].map(riskClass => {
                const count = summary.by_risk_class[riskClass as keyof typeof summary.by_risk_class];
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          {(() => {
            ...
            const criticalAssets = assets.filter((a: any) => a.business_criticality === 'critical');
            ...
            const kemUsage = protectedAssets.reduce((acc: any, a: any) => {
              const kem = a.encryption_profile?.kem || 'Unknown';
              acc[kem] = (acc[kem] || 0) + 1;
              return acc;
            }, {});
            
            const signatureUsage = protectedAssets.reduce((acc: any, a: any) => {
            ...
            const recentlyProtected = protectedAssets.filter((a: any) => {
            ...
            const highRiskUnprotected = unprotectedAssets.filter((a: any) => 
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ 
                    ...
                    <th style={{ padding: '0.75rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: '600', color: '#666', textTransform: 'uppercase' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredAssets.map(asset => (
                    <tr 
                      ...
                      onClick={() => navigate(`/pqc-risk/assets/${asset.id}`)}
                    >
                      ...
                      <td style={{ padding: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          ...
                          {asset.pqc_risk_score !== undefined && (
                            <div style={{
                              width: '60px',
                              height: '4px',
                              background: '#f3f4f6',
                              ...
                              <div style={{
                                height: '100%',
                                width: `${asset.pqc_risk_score}%`,
                                background: getRiskClassColor(asset.risk_class),
                                transition: 'width 0.3s'
                              }} />
                            </div>
                          )}
                      ...
                      <td style={{ padding: '0.75rem', fontSize: '0.9rem' }}>{asset.name}</td>
                      <td style={{ padding: '0.75rem', fontSize: '0.9rem', color: '#666' }}>{asset.asset_type}</td>
                      <td style={{ padding: '0.75rem', fontSize: '0.9rem', color: '#666' }}>{formatEnumValue(asset.environment)}</td>
                      <td style={{ padding: '0.75rem', fontSize: '0.9rem', color: '#666' }}>{asset.owner}</td>
                      <td style={{ padding: '0.75rem' }}>
                        <span style={{
                          ...
                          fontSize: '0.75rem',
                          fontWeight: '600',
                          background: asset.aqv === 5 ? '#fee2e2' : '#f3f4f6',
                          color: asset.aqv === 5 ? '#ef4444' : '#666'
                        }}>
                          {asset.aqv ?? 'N/A'}
                        </span>
                      </td>
                      <td style={{ padding: '0.75rem' }}>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/pqc-risk/assets/${asset.id}`);
                          }}
                          style={{
                            padding: '0.25rem 0.75rem',
                            background: '#3b82f6',
