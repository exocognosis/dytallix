...
export default function StoragePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setError('Failed to load storage encryptions statistics from the server.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-6 lg:p-8 space-y-6">
            ...
            {loading ? (
                <div className="text-white/40 text-center animate-pulse p-12">Loading storage encryption metrics...</div>
            ) : (
                <>
                    {/* Metrics Row */}
                                ...
                                <tbody>
                                    {storageByTenant.map((tenant) => (
                                        <tr key={tenant.name}>
                                            ...
                                            <td className="text-white/50">{tenant.lastRotation || new Date().toISOString().split('T')[0]}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
