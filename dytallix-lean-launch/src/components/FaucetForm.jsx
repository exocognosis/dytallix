...
const FaucetForm = () => {
  ...
  useEffect(() => {
    ...
    if (savedCooldowns) {
      try { const parsed = JSON.parse(savedCooldowns); const now = Date.now(); const active = {}; Object.keys(parsed).forEach(t => { if (parsed[t] > now) active[t] = parsed[t] }); setCooldowns(active) } catch {}
    }
  ...
  useEffect(() => { try { const meta = loadMeta(); if (meta?.address) { setAddress(meta.address); setConnected(true); return } } catch {}; setConnected(false) }, [])
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setMessage('')
    if (!address.trim() || !isBech32(address.trim())) { setMessage('Please enter a valid Dytallix bech32 address'); setMessageType('error'); return }
    if (isOnCooldown(selectedToken)) { const remaining = getCooldownMinutes(selectedToken); setMessage(`Please wait ${remaining} more minutes before requesting ${selectedToken} again.`); setMessageType('error'); return }
    setIsLoading(true)
    try {
