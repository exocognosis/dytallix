...
start_node() {
  ...
  (
    ...
    if [ ! -d target ] || [ -z "$(ls -A target/debug 2>/dev/null || true)" ]; then
      ...
      cargo build --quiet --bin dytallix-lean-node >"$LOG_DIR/node.build.out" 2>"$LOG_DIR/node.build.err" || {
        log "Node build failed; see $LOG_DIR/node.build.err"; exit 1; }
    fi

    # Run node (explicit bin since package defines multiple binaries)
    cargo run --quiet --bin dytallix-lean-node >"$LOG_DIR/node.out" 2>"$LOG_DIR/node.err" &
    write_pid node $!
  )
