...
fn write_dummy_configs(home: &str, moniker: &str, chain_id: &str) -> Result<()> {
    ...
    std::fs::write(
        format!("{cfg_dir}/app.toml"),
        format!("moniker = '{moniker}'\n"),
    )?;
    std::fs::write(
...
async fn main() -> Result<()> {
    ...
    match cli.command.unwrap_or(Cmd::Start {
        ...
        Cmd::Init {
            ...
            ensure_dir(&home)?;
            write_dummy_configs(&home, &moniker, &chain_id)?;
            println!("Initialized {moniker} chain_id={chain_id} home={home}");
        }
        Cmd::Start { home } => {
            ensure_dir(&home)?;
            run_node(&home).await?;
