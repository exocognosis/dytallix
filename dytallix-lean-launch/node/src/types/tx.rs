...
impl Tx {
    ...
    pub fn validate(&self, expected_chain_id: &str) -> Result<()> {
        if self.chain_id != expected_chain_id {
            return Err(anyhow!("invalid chain_id: expected {}, got {}", expected_chain_id, self.chain_id));
        }
...
impl SignedTx {
    ...
    pub fn verify(&self) -> Result<()> {
        if self.algorithm != ActivePQC::ALG { 
            return Err(anyhow!("unsupported algorithm: expected {}, got {}", ActivePQC::ALG, self.algorithm)); 
        }
        if self.version != 1 { 
            return Err(anyhow!("unsupported version: expected 1, got {}", self.version)); 
        }
        let bytes = canonical_json(&self.tx)?; 
        let hash = sha3_256(&bytes);
        let sig = B64.decode(&self.signature)
            .map_err(|e| anyhow!("invalid signature encoding: {}", e))?; 
        let pk = B64.decode(&self.public_key)
...
impl ValidationError {
    pub fn http_status(&self) -> u16 {
        match self {
            ...
            | ValidationError::DuplicateTransaction => 409, // Conflict
...
mod tests {
    ...
    fn test_validation_error_http_status() {
        assert_eq!(ValidationError::InvalidChainId { expected: "test".into(), got: "wrong".into() }.http_status(), 422);
        assert_eq!(ValidationError::InvalidNonce { expected: 1, got: 2 }.http_status(), 409);
        assert_eq!(ValidationError::InvalidSignature.http_status(), 422);
        assert_eq!(ValidationError::InsufficientFunds { required: 100, available: 50 }.http_status(), 422);
    ...
    fn test_validation_error_json() {
        let err = ValidationError::InvalidNonce { expected: 5, got: 3 };
