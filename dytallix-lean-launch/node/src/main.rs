...
use dytallix_lean_node::runtime::emission::{EmissionConfig, EmissionEngine};
...
fn load_emission_engine(storage: Arc<Storage>, state: Arc<Mutex<State>>) -> EmissionEngine {
    if let Ok(path) = std::env::var("DYT_EMISSION_CONFIG") {
        let config_path = path.trim().to_string();
        if !config_path.is_empty() {
            match fs::read_to_string(&config_path) {
                Ok(contents) => match serde_json::from_str::<EmissionConfig>(&contents) {
                    Ok(cfg) => {
                        println!("Using emission config from {config_path}");
                        return EmissionEngine::new_with_config(storage, state, cfg);
                    }
                    Err(e) => {
                        eprintln!("Failed to parse emission config {config_path}: {e}");
                    }
                },
                Err(e) => {
                    eprintln!("Failed to read emission config {config_path}: {e}");
                }
            }
        }
    }

    EmissionEngine::new(storage, state)
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let ctx = RpcContext {
        ...
        emission: Arc::new(Mutex::new(load_emission_engine(
            storage.clone(),
            state.clone(),
        ))),
    ...
    let mut app = Router::new()
        ...
        // Staking transaction routes required by evidence scripts
