...
fn test_remainder_allocation_stable() {
    ...
    if let Some(event) = engine.get_event(1) {
        ...
        assert_eq!(actual_bridge, expected_bridge, "Bridge operations should get remainder");
        
        // Verify no emission is lost
        let pool_sum: u128 = event.pools.values().sum();
        assert_eq!(pool_sum, total, "No emission should be lost due to rounding");
    }
}
...
fn test_zero_stake_carry_and_first_application() {
    ...
    let expected_reward_index = (1500 * dytallix_lean_node::runtime::staking::REWARD_SCALE) / 1_000_000;
...
fn test_reward_index_precision() {
    ...
    let expected = (1_000_000 * dytallix_lean_node::runtime::staking::REWARD_SCALE) / 1_000_000_000_000;
    ...
    assert!(error < TOLERANCE, "Precision error {} exceeds tolerance {}", error, TOLERANCE);
}
...
fn test_event_persistence_and_idempotency() {
    ...
    let mut engine = EmissionEngine::new_with_config(storage.clone(), state.clone(), config.clone());
...
fn test_circulating_supply_tracking() {
    ...
    for height in 1..=5 {
        if let Some(event) = engine.get_event(height) {
            assert!(event.circulating_supply > last_supply, "Circulating supply should increase");
...
fn test_bootstrap_emission_calculation() {
    ...
    if let Some(event) = engine.get_event(1) {
        ...
        assert!(event.circulating_supply > 0, "Circulating supply should increase");
    }
    ...
    if let Some(event) = engine.get_event(2) {
        ...
        assert!(event.total_emitted > 0, "Calculated emission should be positive");
