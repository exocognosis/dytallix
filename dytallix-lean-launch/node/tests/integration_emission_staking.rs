...
fn test_emission_staking_integration() {
    ...
    let expected_reward_index = (total_staking_rewards * dytallix_lean_node::runtime::staking::REWARD_SCALE) / validator_stake;
    assert_eq!(final_reward_index, expected_reward_index);
    
    // Verify reward accumulation is meaningful
    assert!(final_reward_index > 0, "Reward index should increase over {} blocks", num_blocks);
    assert!(total_staking_rewards > 0, "Total staking rewards should be positive");
    
    // Test precision - calculate potential claimable rewards for a delegator
    let delegator_stake = 100_000_000_000u128; // 100K DGT
    let claimable_rewards = (delegator_stake * final_reward_index) / dytallix_lean_node::runtime::staking::REWARD_SCALE;
    ...
    assert!(difference <= 1, "Delegator reward calculation error {} should be ≤ 1 unit", difference);
}
...
fn test_zero_stake_then_delegation() {
    ...
    let expected_reward_index = (accumulated_rewards * dytallix_lean_node::runtime::staking::REWARD_SCALE) / validator_stake;
    ...
    assert!(final_reward_index > new_reward_index, "Reward index should continue to increase");
}
...
fn test_multiple_stake_changes() {
    ...
    assert_eq!(final_pending, 0);
    assert!(final_reward_index > reward_index_1, "Reward index should continue increasing");
    ...
    assert!(increase_per_block_2 < increase_per_block_1, "Reward rate should decrease with higher stake");
}
...
fn test_emission_event_consistency() {
    ...
    for height in 1..=20 {
        if let Some(event) = emission.get_event(height) {
            ...
            assert!(block_rewards_pct >= 59 && block_rewards_pct <= 61, "Block rewards should be ~60% at height {}", height);
            assert!(staking_rewards_pct >= 24 && staking_rewards_pct <= 26, "Staking rewards should be ~25% at height {}", height);
            assert!(ai_incentives_pct >= 9 && ai_incentives_pct <= 11, "AI incentives should be ~10% at height {}", height);
            assert!(bridge_ops_pct >= 4 && bridge_ops_pct <= 6, "Bridge ops should be ~5% at height {}", height);
            
            // Verify circulating supply increases monotonically
            if height > 1 {
                if let Some(prev_event) = emission.get_event(height - 1) {
                    assert!(event.circulating_supply > prev_event.circulating_supply, 
                           "Circulating supply should increase at height {}", height);
                    assert_eq!(event.circulating_supply, 
...
fn test_staking_rewards_precision() {
    ...
    for delegator_stake in test_stakes {
        let claimable = (delegator_stake * final_reward_index) / dytallix_lean_node::runtime::staking::REWARD_SCALE;
        
        // Verify claimable amount is reasonable
        assert!(claimable > 0, "Delegator with {} stake should earn some rewards", delegator_stake);
        
        // Verify precision - claimable should be proportional to stake
        let stake_ratio = (delegator_stake as f64) / (total_stake as f64);
        let expected_ratio = (claimable as f64) / (final_reward_index as f64) * (dytallix_lean_node::runtime::staking::REWARD_SCALE as f64);
        
        let ratio_error = (stake_ratio - expected_ratio).abs();
        assert!(ratio_error < TOLERANCE, "Stake ratio precision error {} exceeds tolerance for stake {}", ratio_error, delegator_stake);
    }
}