...
fn test_receipt_version_consistency() {
    ...
    let transactions = vec![
        ...
        Transaction::new("tx2".to_string(), "alice".to_string(), "bob".to_string(), 500, 3_000, 1, Some("sig".to_string())),
    ];
...
fn test_json_schema_validation() {
    ...
    for (field_name, expected_type) in required_fields {
        assert!(json_value.get(field_name).is_some(), "Missing field: {}", field_name);
        
        let field_value = &json_value[field_name];
        match expected_type {
            "number" => assert!(field_value.is_number(), "Field {} should be number", field_name),
            "string" => assert!(field_value.is_string(), "Field {} should be string", field_name),
            "boolean" => assert!(field_value.is_boolean(), "Field {} should be boolean", field_name),
