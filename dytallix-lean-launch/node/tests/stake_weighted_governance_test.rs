...
fn test_stake_weighted_tally_quorum_not_met() {
    ...
    governance.deposit(100, "depositor", proposal_id, 1_000_000_000, "udgt").unwrap();
    
    // Cast vote with insufficient participation (20% < 33.33% quorum)
    governance.vote(150, "voter1", proposal_id, VoteOption::Yes).unwrap();
...
fn test_stake_weighted_tally_veto_triggered() {
    ...
    governance.vote(150, "voter1", proposal_id, VoteOption::Yes).unwrap();
    governance.vote(150, "voter2", proposal_id, VoteOption::NoWithVeto).unwrap();
...
fn test_stake_weighted_tally_successful_pass() {
    ...
    governance.vote(150, "voter1", proposal_id, VoteOption::Yes).unwrap();
    governance.vote(150, "voter2", proposal_id, VoteOption::No).unwrap();
    governance.vote(150, "voter3", proposal_id, VoteOption::Abstain).unwrap();
...
fn test_parameter_change_events() {
    ...
    let param_change_event = events.iter().find(|event| {
        matches!(event, GovernanceEvent::ParameterChanged { .. })
    });
    
    assert!(param_change_event.is_some());
    if let Some(GovernanceEvent::ParameterChanged { key, old_value, new_value }) = param_change_event {
...
fn test_stake_weighted_vs_token_balance_difference() {
    ...
    // Add deposit to reach voting period
    governance.deposit(100, "rich_user", proposal_id, 1_000_000_000, "udgt").unwrap();
    
    // Vote should succeed but have zero weight
    governance.vote(150, "rich_user", proposal_id, VoteOption::Yes).unwrap();
