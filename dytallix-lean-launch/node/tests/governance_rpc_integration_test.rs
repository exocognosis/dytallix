...
async fn governance_rpc_happy_path_test() {
    ...
    let submit_response = dytallix_lean_node::rpc::gov_submit_proposal(
    ...
    let _deposit_response = dytallix_lean_node::rpc::gov_deposit(
        axum::Extension(ctx.clone()),
    ...
    let execute_response = dytallix_lean_node::rpc::gov_execute(
        axum::Extension(ctx.clone()),
