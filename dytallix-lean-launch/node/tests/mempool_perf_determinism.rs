...
async fn test_deterministic_ordering_across_instances() {
    ...
    for (tx1, tx2) in snapshot1.iter().zip(snapshot2.iter()) {
        assert_eq!(tx1.hash, tx2.hash, "Transaction order differs between instances");
        assert_eq!(tx1.gas_price, tx2.gas_price);
        assert_eq!(tx1.nonce, tx2.nonce);
    }
    
    println!("✅ Deterministic ordering verified across {} transactions", snapshot1.len());
}
...
async fn test_priority_key_determinism() {
    ...
    let tx1 = create_test_transaction(
        "hash_a", "sender1", "receiver", 1000, 100, 5, 21000, 2000
    );
    let tx2 = create_test_transaction(
        "hash_b", "sender2", "receiver", 1000, 100, 3, 21000, 2000
    );
    let tx3 = create_test_transaction(
        "hash_c", "sender3", "receiver", 1000, 100, 5, 21000, 1500
    );
    ...
    let tx4 = create_test_transaction(
        "hash_d", "sender4", "receiver", 1000, 100, 5, 21000, 2000
    );
...
async fn test_concurrent_access_simulation() {
    ...
    for i in 0..final_snapshot.len().saturating_sub(1) {
        ...
        assert!(
            ...
            (current.gas_price == next.gas_price && current.nonce == next.nonce && current.hash <= next.hash),
            "Ordering constraint violated at position {}: current({}, {}, {}) vs next({}, {}, {})",
            i, current.gas_price, current.nonce, current.hash, next.gas_price, next.nonce, next.hash
        );
    }
    
    println!(
        "✅ Concurrent access simulation passed: bulk {:?}, interleaved {:?}, final size {}",
        bulk_duration, interleaved_duration, final_snapshot.len()
    );
}