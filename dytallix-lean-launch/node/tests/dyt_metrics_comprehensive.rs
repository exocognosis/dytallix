...
async fn test_dyt_metrics_exposure() {
    ...
    metrics.dyt_blocks_produced_total.with_label_values(&validator_label).inc();
    assert_eq!(metrics.dyt_blocks_produced_total.with_label_values(&validator_label).get(), 1);
    
    metrics.dyt_validator_missed_blocks_total.with_label_values(&validator_label).inc();
    assert_eq!(metrics.dyt_validator_missed_blocks_total.with_label_values(&validator_label).get(), 1);
    
    metrics.dyt_validator_voting_power.with_label_values(&validator_label).set(100.0);
    assert_eq!(metrics.dyt_validator_voting_power.with_label_values(&validator_label).get(), 100.0);
    
    // Test emission metrics
    let pool_type_label = ["staking"];
    metrics.dyt_emission_pool_amount.with_label_values(&pool_type_label).set(1000000.0);
    assert_eq!(metrics.dyt_emission_pool_amount.with_label_values(&pool_type_label).get(), 1000000.0);
...
async fn test_prometheus_format_dyt_metrics() {
    ...
    metrics.dyt_blocks_produced_total.with_label_values(&validator_labels).inc_by(5);
    metrics.dyt_validator_voting_power.with_label_values(&validator_labels).set(150.0);
    
    // Record emission metrics
    let pool_labels = ["rewards"];
    metrics.dyt_emission_pool_amount.with_label_values(&pool_labels).set(500000.0);
    ...
    assert!(output.contains(r#"dyt_blocks_produced_total{validator="validator-1"} 5"#));
    assert!(output.contains(r#"dyt_validator_voting_power{validator="validator-1"} 150"#));
    assert!(output.contains(r#"dyt_emission_pool_amount{pool_type="rewards"} 500000"#));
...
async fn test_metrics_endpoint_dyt_metrics() {
    ...
    metrics.dyt_blocks_produced_total.with_label_values(&validator_labels).inc_by(10);
    
    // The server start would bind to a port, but we can't easily test the HTTP endpoint
    // in this unit test without more complex setup. The creation and metrics recording
    // tests above cover the essential functionality.
...
/// Test metrics with no-op implementation when feature is disabled
...
async fn test_metrics_disabled_no_ops() {
    ...
    // These calls should compile and not panic
    ...
    println!("✅ No-op metrics implementation works correctly");
}
...
async fn test_full_metrics_recording_flow() {
    ...
    metrics.dyt_blocks_produced_total.with_label_values(&validator).inc();
    metrics.dyt_validator_voting_power.with_label_values(&validator).set(1000.0);
    ...
    metrics.dyt_emission_pool_amount.with_label_values(&staking_pool).set(10000000.0);
    metrics.dyt_emission_pool_amount.with_label_values(&rewards_pool).set(500000.0);
    
    // Verify final state
    assert_eq!(metrics.dyt_block_height.get(), 100);
    assert_eq!(metrics.dyt_blocks_produced_total.with_label_values(&validator).get(), 1);
    assert_eq!(metrics.dyt_tps.get(), 125.0);
    assert_eq!(metrics.dyt_mempool_size.get(), 1000);
    assert_eq!(metrics.dyt_validator_voting_power.with_label_values(&validator).get(), 1000.0);
    assert_eq!(metrics.dyt_emission_pool_amount.with_label_values(&staking_pool).get(), 10000000.0);
