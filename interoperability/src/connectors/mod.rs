...
pub use ethereum::{EthereumConnector, EthereumConfig, EthereumAddress, EthereumTxHash, EthereumBlock};
pub use cosmos::{CosmosConnector, CosmosConfig, CosmosAddress, CosmosTxHash, CosmosBlock};
pub use polkadot::{PolkadotConnector, PolkadotConfig, PolkadotAddress, PolkadotTxHash, PolkadotBlock, PolkadotChainType};
...
pub trait ChainConnector {
    ...
    fn new(config: Self::Config) -> Result<Self, BridgeError> where Self: Sized;
...
impl ConnectorManager {
    ...
    pub fn get_ethereum_connector_mut(&mut self, chain_name: &str) -> Option<&mut EthereumConnector> {
    ...
    pub async fn process_transfer(
        ...
        println!("🌉 Processing transfer from {} to {}", source_chain, dest_chain);
        
        // Determine chain types and execute appropriate transfer logic
        match (self.get_chain_type(source_chain), self.get_chain_type(dest_chain)) {
            (ChainType::Ethereum, ChainType::Dytallix) => {
                self.ethereum_to_dytallix_transfer(source_chain, bridge_tx).await
            },
            (ChainType::Cosmos, ChainType::Dytallix) => {
                self.cosmos_to_dytallix_transfer(source_chain, bridge_tx).await
            },
            (ChainType::Polkadot, ChainType::Dytallix) => {
                self.polkadot_to_dytallix_transfer(source_chain, bridge_tx).await
            },
            (ChainType::Dytallix, ChainType::Ethereum) => {
                self.dytallix_to_ethereum_transfer(dest_chain, bridge_tx).await
            },
            (ChainType::Dytallix, ChainType::Cosmos) => {
                self.dytallix_to_cosmos_transfer(dest_chain, bridge_tx).await
            },
            (ChainType::Dytallix, ChainType::Polkadot) => {
                self.dytallix_to_polkadot_transfer(dest_chain, bridge_tx).await
            },
            _ => Err(BridgeError::UnsupportedChain(format!("Transfer from {} to {} not supported", source_chain, dest_chain))),
        }
    }
    ...
    async fn ethereum_to_dytallix_transfer(
        ...
        let connector = self.get_ethereum_connector(source_chain)
            .ok_or_else(|| BridgeError::UnsupportedChain(source_chain.to_string()))?;
        
        let tx_hash = connector.lock_asset(&bridge_tx.asset, &bridge_tx.dest_address, bridge_tx).await?;
    ...
    async fn cosmos_to_dytallix_transfer(
        ...
        let connector = self.get_cosmos_connector_mut(source_chain)
            .ok_or_else(|| BridgeError::UnsupportedChain(source_chain.to_string()))?;
        
        let tx_hash = connector.ibc_transfer(&bridge_tx.asset, &bridge_tx.dest_address, bridge_tx).await?;
    ...
    async fn polkadot_to_dytallix_transfer(
        ...
        let connector = self.get_polkadot_connector(source_chain)
            .ok_or_else(|| BridgeError::UnsupportedChain(source_chain.to_string()))?;
    ...
    async fn dytallix_to_ethereum_transfer(
        ...
        let connector = self.get_ethereum_connector_mut(dest_chain)
        ...
        let tx_hash = connector.mint_wrapped_asset(&wrapped_asset, &bridge_tx.dest_address).await?;
    ...
    async fn dytallix_to_cosmos_transfer(
        ...
        let connector = self.get_cosmos_connector_mut(dest_chain)
            .ok_or_else(|| BridgeError::UnsupportedChain(dest_chain.to_string()))?;
    ...
    async fn dytallix_to_polkadot_transfer(
        ...
        let connector = self.get_polkadot_connector(dest_chain)
            .ok_or_else(|| BridgeError::UnsupportedChain(dest_chain.to_string()))?;
