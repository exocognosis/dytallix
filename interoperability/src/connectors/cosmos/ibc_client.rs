...
impl CosmosIbcClient {
    ...
    pub async fn connect(&mut self) -> Result<(), BridgeError> {
        ...
        if self.rpc_url.is_empty() || self.grpc_url.is_empty() {
            return Err(BridgeError::InvalidChain("Invalid RPC or gRPC URL".to_string()));
        }
    ...
    pub async fn send_ibc_transfer(&mut self, packet: IbcPacket) -> Result<CosmosTxHash, BridgeError> {
        if !self.connected {
            return Err(BridgeError::ConnectionFailed("Not connected to Cosmos network".to_string()));
        }
        ...
        println!("   From: {} -> {}", packet.source_channel, packet.dest_channel);
    ...
    pub async fn query_balance(&mut self, address: &str, denom: &str) -> Result<u64, BridgeError> {
        ...
        if !address.starts_with("cosmos") && !address.starts_with("osmo") {
            return Err(BridgeError::InvalidAddress(format!("Invalid Cosmos address: {}", address)));
        }
    ...
    pub async fn get_channel_info(&mut self, port: &str, channel: &str) -> Result<ChannelInfo, BridgeError> {
        if !self.connected {
            return Err(BridgeError::ConnectionFailed("Not connected to Cosmos network".to_string()));
        }
    ...
    pub async fn deploy_bridge_contract(&mut self, code_bytes: &[u8], init_msg: &str) -> Result<String, BridgeError> {
        if !self.connected {
            return Err(BridgeError::ConnectionFailed("Not connected to Cosmos network".to_string()));
        }
    ...
    pub async fn execute_bridge_contract(&mut self, contract_addr: &str, execute_msg: &str) -> Result<CosmosTxHash, BridgeError> {
        if !self.connected {
            return Err(BridgeError::ConnectionFailed("Not connected to Cosmos network".to_string()));
        }
