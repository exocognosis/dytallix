...
impl CosmosRelayer {
    ...
    pub async fn relay_packet(&mut self, packet: IbcPacket) -> Result<RelayResult, BridgeError> {
        println!("📨 Relaying IBC packet from {}:{} to {}:{}", 
    ...
    pub fn cleanup_old_packets(&mut self, max_age_seconds: u64) {
        ...
        self.pending_packets.retain(|packet| {
            current_time - packet.last_attempt < max_age_seconds
        });
    }
...
mod tests {
    ...
    async fn test_packet_acknowledgement() {
        ...
        assert_eq!(relayer.pending_packets[0].status, PacketStatus::Acknowledged);
    }
}
