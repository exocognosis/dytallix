...
impl TokenomicsGovernanceExample {
    ...
    pub fn scenario_increase_emission_rate(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "Current emission rate: {} DRT per block",
            current_params.base_emission_rate
        );
        println!(
            "Current max emission rate: {} DRT per block",
            current_params.max_emission_rate
        );
        println!();
        ...
        if vote_result.yes_votes > vote_result.no_votes {
            ...
            if let Some(tokenomics_proposal) =
                self.governance.execute_tokenomics_proposal(proposal_id)?
            {
                // Submit to emission controller
                self.emission_controller
                    .submit_proposal(proposal_id, tokenomics_proposal)?;
                ...
                println!(
                    "✓ Emission rate updated to: {} DRT per block",
    ...
    pub fn scenario_update_emission_parameters(
        &mut self,
        ...
        println!(
            "New adjustment factor: {}% (basis points: {})",
            new_params.adjustment_factor as f64 / 100.0,
    ...
    pub fn scenario_adaptive_emission(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "  Staker pool: {} DRT",
            self.emission_controller.staker_pool_balance()
        );
...
impl std::fmt::Display for TokenomicsSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "  Current Emission: {} DRT per block",
