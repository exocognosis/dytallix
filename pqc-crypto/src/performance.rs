...
impl PQCPerformanceBenchmark {
    ...
    pub fn run_comprehensive_benchmarks(&mut self) -> Result<PerformanceAnalysis, Box<dyn std::error::Error>> {
    ...
    fn benchmark_algorithm(&mut self, name: &str, algorithm: &SignatureAlgorithm) -> Result<PQCBenchmarkResults, Box<dyn std::error::Error>> {
    ...
    fn single_operation_benchmark(&mut self, algorithm: &SignatureAlgorithm) -> Result<(Duration, Duration, Duration, usize, usize), Box<dyn std::error::Error>> {
        ...
        let signature = self.pqc_manager.sign_bridge_payload(&payload, "ethereum", &validator_id)?;
        let signature_time = signature_start.elapsed();
        
        // Verification benchmark
        let verification_start = Instant::now();
        let _is_valid = self.pqc_manager.verify_bridge_signature(&signature, &payload)?;
    ...
    fn estimate_algorithm_gas_cost(&self, algorithm: &SignatureAlgorithm, signature_size: usize) -> Result<u64, Box<dyn std::error::Error>> {
    ...
    fn estimate_gas_costs(&self, results: &[PQCBenchmarkResults]) -> Result<Vec<GasCostEstimation>, Box<dyn std::error::Error>> {
        let networks = vec![
    ...
    fn generate_recommendations(&self, results: &[PQCBenchmarkResults], gas_estimations: &[GasCostEstimation]) -> Vec<String> {
        ...
        if let Some(smallest_sig) = smallest_signature {
            recommendations.push(format!(
                ...
                smallest_sig.algorithm,
                smallest_sig.signature_size
            ));
        }
        ...
        if let Some(lowest_gas_cost) = lowest_gas {
            recommendations.push(format!(
                ...
                lowest_gas_cost.algorithm,
                lowest_gas_cost.estimated_gas_cost
            ));
        }
        
        // Find lowest cost network
        let lowest_cost_network = gas_estimations.iter().min_by(|a, b| a.cost_in_usd.partial_cmp(&b.cost_in_usd).unwrap());
        if let Some(lowest_cost) = lowest_cost_network {
            recommendations.push(format!(
                "Most cost-effective network: {} (${:.4} USD)",
                lowest_cost.network,
                lowest_cost.cost_in_usd
            ));
        }
        
        // General optimization recommendations
        recommendations.push("Consider batching multiple signatures for better gas efficiency".to_string());
        recommendations.push("Implement signature aggregation where possible to reduce verification overhead".to_string());
        recommendations.push("Use Falcon1024 for latency-critical applications due to faster verification".to_string());
        recommendations.push("Use Dilithium5 for balanced security and performance".to_string());
        recommendations.push("Consider SPHINCS+ only for maximum long-term security requirements".to_string());
    ...
    pub fn print_performance_report(&self, analysis: &PerformanceAnalysis) {
        ...
        println!("{:-<100}", "");
        println!("{:<12} {:<15} {:<15} {:<15} {:<12} {:<12} {:<8}",
                "Algorithm", "Key Gen (ms)", "Sign (ms)", "Verify (ms)", 
                "Sig Size", "Gas Cost", "Ops/sec");
        println!("{:-<100}", "");
        
        for result in &analysis.benchmarks {
            println!("{:<12} {:<15.2} {:<15.2} {:<15.2} {:<12} {:<12} {:<8.1}",
                    result.algorithm,
        ...
        // Summary
        println!("\n📈 Summary:");
        println!("  • Total algorithms benchmarked: {}", analysis.benchmarks.len());
        println!("  • Best overall algorithm: {}", analysis.optimal_algorithm);
        
        if let Some(best) = analysis.benchmarks.iter().find(|r| r.algorithm == analysis.optimal_algorithm) {
            println!("  • Best algorithm signature time: {:.2}ms", best.signature_time.as_millis());
            println!("  • Best algorithm verification time: {:.2}ms", best.verification_time.as_millis());
    ...
    pub fn export_results(&self, analysis: &PerformanceAnalysis, filename: &str) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_gas_cost_estimation() {
        ...
        let dilithium_gas = benchmark.estimate_algorithm_gas_cost(&SignatureAlgorithm::Dilithium5, 2420).unwrap();
        let falcon_gas = benchmark.estimate_algorithm_gas_cost(&SignatureAlgorithm::Falcon1024, 690).unwrap();
        let sphincs_gas = benchmark.estimate_algorithm_gas_cost(&SignatureAlgorithm::SphincsSha256128s, 7856).unwrap();
