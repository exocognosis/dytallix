...
fn validate_signed_tx(
    ...
    // For gasless anchoring: skip nonce and balance checks for zero-fee Data-only transactions
    let is_data_only = signed_tx.tx.msgs.iter().all(|m| matches!(m, Msg::Data { .. }));
    let is_gasless = signed_tx.tx.fee == 0 && is_data_only;
    
    if is_gasless {
        eprintln!("[INFO] Gasless anchoring transaction detected - skipping nonce and balance checks");
        return Ok(());
    }
