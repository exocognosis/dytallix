...
impl Tx {
    pub fn new(
        ...
        // Allow zero fees for Data messages (gasless anchoring)
        let is_data_only = msgs.iter().all(|m| matches!(m, Msg::Data { .. }));
        if fee == 0 && !is_data_only {
            return Err(anyhow!("fee cannot be zero for token transfer transactions"));
        }
    ...
    pub fn validate(&self, expected_chain_id: &str) -> Result<()> {
        ...
        // Allow zero fees for Data messages (gasless anchoring)
        let is_data_only = self.msgs.iter().all(|m| matches!(m, Msg::Data { .. }));
        if self.fee == 0 && !is_data_only {
            return Err(anyhow!("fee cannot be zero for token transfer transactions"));
        }
