...
class FraudDetector:
    ...
    async def analyze_transaction(
        self, transaction: Dict[str, Any], historical_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.detector and hasattr(self.detector, "analyze_transaction"):
                return await self.detector.analyze_transaction(
    ...
    def _extract_features(
        self, transaction: Dict, historical_data: List[Dict]
    ) -> np.ndarray:
        ...
        if historical_data:
            recent_txs = [
                ...
                if tx.get("timestamp", 0) > timestamp - 3600
            ]  # Last hour
    ...
    def _calculate_fraud_score(self, features: np.ndarray) -> float:
        ...
        try:
            ...
            if self.scaler_fitted:
                ...
                # Use raw features when scaler is not fitted
    ...
    def _identify_risk_factors(
        self, transaction: Dict, features: np.ndarray, fraud_score: float
    ) -> List[str]:
    ...
    def _calculate_round_trip_score(
        ...
        for tx in historical_data:
            ...
            if (
                tx_from == to_addr
                and tx_to == from_addr
                and abs(tx_amount - amount) / amount < 0.1  # Similar amounts
                and abs(tx_timestamp - timestamp) < 3600
            ):  # Within 1 hour
    ...
    def _generate_recommendation(
        self, fraud_score: float, risk_factors: List[str]
    ) -> str:
    ...
    async def update_model(self, training_data: List[Dict]) -> bool:
        ...
        try:
            ...
            for tx in training_data:
                features = self._extract_features(tx, tx.get("historical_data", []))
                feature_list.append(features[0])

            if feature_list:
                feature_matrix = np.vstack(feature_list)

                # Fit scaler and model once using training data
