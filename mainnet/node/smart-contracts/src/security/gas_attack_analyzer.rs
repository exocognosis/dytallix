...
impl GasAttackAnalyzer {
    ...
    fn detect_gas_griefing(
        ...
        // Both boundaries are strict: efficiency below 60% and gas used above 100,000.
...
mod tests {
    ...
    async fn test_gas_griefing_detection() {
        ...
        let result = ExecutionResult {
            success: true,
            return_data: vec![],
            gas_used: 100_001, // Low efficiency and above the strict eligibility boundary.
            gas_remaining: 899_999,
            state_changes: vec![],
            events: vec![],
            ai_analysis: None,
        };
    ...
    fn gas_griefing_execution(gas_used: u64) -> (ContractCall, ExecutionResult) {
    ...
    #[tokio::test]
    async fn test_gas_griefing_efficiency_boundary() {
        // Every case exceeds the eligibility boundary of 100,000 gas.
        for (gas_used, expected_count) in [(599_999, 1), (600_000, 0), (600_001, 0)] {
            let mut analyzer = GasAttackAnalyzer::new();
            let (call, result) = gas_griefing_execution(gas_used);
            let findings = analyzer.analyze_execution(&call, &result).await;
            let count = findings
                .iter()
                .filter(|finding| finding.id.starts_with("GAS-GRIEF-EXEC-"))
                .count();
            assert_eq!(count, expected_count, "gas used: {gas_used}");
        }
    }

    #[tokio::test]
    async fn test_gas_griefing_eligibility_boundary() {
        // Every case has efficiency below 60%.
        for (gas_used, expected_count) in [(99_999, 0), (100_000, 0), (100_001, 1)] {
            let mut analyzer = GasAttackAnalyzer::new();
            let (call, result) = gas_griefing_execution(gas_used);
            let findings = analyzer.analyze_execution(&call, &result).await;
            let count = findings
