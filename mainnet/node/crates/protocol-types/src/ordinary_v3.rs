//! Provisional ordinary-v3 signing codec for local engineering only.
//! This module has no consensus activation, signature verifier, or fee policy.
//! The separate ordinary-v2 module and its accepted bytes remain unchanged.
//! Binary bytes are authoritative. Serde representations are client views only.
//! Current authority, expiry, fee outcomes and action-class rules require state.
use crate::ordinary as v2;
...
pub const GOVERNANCE_ACTION_PREFIX: &[u8] = b"DYTALLIX/GOVERNANCE-ACTION-V1\0";
...
pub struct V3Limits {
    pub ordinary: v2::Limits,
    pub max_governance_action_bytes: u32,
}
impl Deref for V3Limits {
    type Target = v2::Limits;
...
impl V3Limits {
    pub fn validate(&self) -> Result<()> {
        ...
        need(
            self.max_governance_action_bytes > 0
                && self.max_governance_action_bytes <= self.max_wire_bytes,
            "explicit governance action bound required",
        )
    }
}
pub type Limits = V3Limits;

/// Digest of a proposed action. Action-class authorization is a consensus rule.
pub fn governance_action_digest(action_class: u16, action_data: &[u8]) -> Result<[u8; 32]> {
    let length = u32::try_from(action_data.len())
        .map_err(|_| WireError("governance action length overflow".into()))?;
    let mut hasher = Sha3_256::new();
    hasher.update(GOVERNANCE_ACTION_PREFIX);
    hasher.update(action_class.to_be_bytes());
...
impl VoteChoice {
    ...
    fn from_code(code: u8) -> Result<Self> {
        match code {
            ...
            4 => Ok(Self::Abstain),
            _ => Err(WireError("unsupported governance vote choice".into())),
        }
    }
}
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
impl<'de> Deserialize<'de> for Action {
    fn deserialize<D: serde::Deserializer<'de>>(
        ...
        Ok(match ActionView::deserialize(deserializer)? {
            ...
            ActionView::GovernanceDeposit {
                proposal_id,
                amount_udgt,
            } => Self::GovernanceDeposit {
                proposal_id,
                amount_udgt,
            },
            ActionView::GovernanceVote {
                proposal_id,
                choice,
            } => Self::GovernanceVote {
                proposal_id,
    ...
    fn action(&mut self, v: &Action, l: &Limits) -> Result<()> {
        ...
        match v {
            ...
            Action::GovernanceProposal {
                ...
                need(
                    *action_digest == governance_action_digest(*action_class, action_data)?,
                    "governance action digest differs",
                )?;
                self.u8(13)?;
                self.u64(*proposal_id)?;
                self.u16(*action_class)?;
                self.blob(action_data, capacity(l.max_governance_action_bytes)?)?;
                self.raw(action_digest)
            }
...
impl<'a> Reader<'a> {
    ...
    fn action(&mut self, l: &Limits) -> Result<Action> {
        ...
        Ok(match self.u8()? {
            ...
            13 => Action::GovernanceProposal {
                proposal_id: self.u64()?,
                action_class: self.u16()?,
                action_data: self
                    .blob(capacity(l.max_governance_action_bytes)?)?
                    .to_vec(),
                action_digest: self.id()?,
            },
            14 => Action::GovernanceDeposit {
                proposal_id: self.u64()?,
                amount_udgt: self.u128()?,
            },
            15 => Action::GovernanceVote {
                proposal_id: self.u64()?,
                choice: VoteChoice::from_code(self.u8()?)?,
            },
