// Package dispatch describes RPC methods without an HTTP or WebSocket implementation.
package dispatch
...
// Transport adapters use these accessors without owning the method metadata.
func (f *RPCFunc) Call(args []reflect.Value) []reflect.Value { return f.f.Call(args) }
func (f *RPCFunc) ArgType(index int) reflect.Type            { return f.args[index] }
func (f *RPCFunc) ArgCount() int                             { return len(f.args) }
func (f *RPCFunc) ArgNames() []string                        { return append([]string(nil), f.argNames...) }
func (f *RPCFunc) WebSocketOnly() bool                       { return f.ws }
