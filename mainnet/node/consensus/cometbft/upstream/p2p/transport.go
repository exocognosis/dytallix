...
// AuthenticatedConn is a framed connection whose remote key was authenticated
// before the CometBFT NodeInfo exchange. The connection owns all record security.
type AuthenticatedConn interface {
	net.Conn
	RemotePubKey() crypto.PubKey
}

// AuthenticatedConnUpgrade replaces the legacy SecretConnection upgrade when
// explicitly installed before Listen. dialedAddr is nil for inbound traffic.
// An error rejects the connection; the transport never retries a legacy path.
type AuthenticatedConnUpgrade func(net.Conn, *NetAddress, time.Duration) (AuthenticatedConn, error)
...
type MultiplexTransport struct {
	...
	nodeInfo             NodeInfo
	nodeKey              NodeKey
	authenticatedUpgrade AuthenticatedConnUpgrade
	resolver             IPResolver
...
// SetAuthenticatedConnUpgrade installs exactly one explicit authenticated
// upgrade before the transport starts. It cannot be cleared to enable fallback.
func (mt *MultiplexTransport) SetAuthenticatedConnUpgrade(upgrade AuthenticatedConnUpgrade) error {
	if upgrade == nil || mt.authenticatedUpgrade != nil || mt.listener != nil {
		return fmt.Errorf("authenticated upgrade must be installed once before Listen")
	}
	mt.authenticatedUpgrade = upgrade
	return nil
}
...
func (mt *MultiplexTransport) Listen(addr NetAddress) error {
	if err := mt.requireBuildProfileTransport(); err != nil {
		return err
	}
...
func (mt *MultiplexTransport) upgrade(
...
) (secretConn AuthenticatedConn, nodeInfo NodeInfo, err error) {
	var upgraded AuthenticatedConn
	defer func() {
		if err != nil {
			if upgraded != nil {
				_ = upgraded.Close()
	...
	if mt.authenticatedUpgrade != nil {
		secretConn, err = mt.authenticatedUpgrade(c, dialedAddr, mt.handshakeTimeout)
	} else {
		secretConn, err = upgradeSecretConn(c, mt.handshakeTimeout, mt.nodeKey.PrivKey)
	}
	upgraded = secretConn
	if err == nil && (secretConn == nil || secretConn.RemotePubKey() == nil) {
		err = fmt.Errorf("authenticated upgrade returned no connection or remote key")
	}
	if err != nil {
		return nil, nil, ErrRejected{
			conn:          c,
			err:           fmt.Errorf("authenticated conn failed: %v", err),
			isAuthFailure: true,
		}
	}
