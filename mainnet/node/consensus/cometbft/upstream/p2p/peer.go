...
// ID uses the authenticated key from the selected record transport.
// Both the legacy and explicitly injected transport implement AuthenticatedConn.
func (pc peerConn) ID() ID {
	return PubKeyToID(pc.conn.(AuthenticatedConn).RemotePubKey())
}
