...
func GenFilePV(keyFilePath, stateFilePath string) *FilePV {
	return NewFilePV(keydefaults.MustGenerate(), keyFilePath, stateFilePath)
}
...
// LoadFilePVCheckedBytes uses bytes already read through a caller's protected
// file-open policy. It does not reopen either file before signing starts.
func LoadFilePVCheckedBytes(keyJSONBytes, stateJSONBytes []byte, keyFilePath, stateFilePath string) (*FilePV, error) {
	var key FilePVKey
	if err := cmtjson.Unmarshal(keyJSONBytes, &key); err != nil {
		return nil, fmt.Errorf("decode validator key: %w", err)
	}
	if key.PrivKey == nil || key.PubKey == nil || !bytes.Equal(key.PrivKey.PubKey().Bytes(), key.PubKey.Bytes()) || !bytes.Equal(key.Address, key.PubKey.Address()) {
		return nil, errors.New("validator key fields disagree")
	}
	var state FilePVLastSignState
	if err := cmtjson.Unmarshal(stateJSONBytes, &state); err != nil {
		return nil, fmt.Errorf("decode validator signing state: %w", err)
	}
