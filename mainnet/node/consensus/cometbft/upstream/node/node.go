...
type Node struct {
	...
	authenticatedTransportError error
	isListening                 bool
...
// WithAuthenticatedTransport installs an explicit Comet TCP authentication and
// record layer before any listener starts. Unsupported transports fail node
// construction. There is no fallback after the callback rejects a connection.
func WithAuthenticatedTransport(upgrade p2p.AuthenticatedConnUpgrade) Option {
	return func(n *Node) {
		if n.authenticatedTransportError != nil {
			return
		}
		transport, ok := n.transport.(*p2p.MultiplexTransport)
		if !ok {
			n.authenticatedTransportError = fmt.Errorf("authenticated transport requires Comet TCP networking")
			return
		}
		n.authenticatedTransportError = transport.SetAuthenticatedConnUpgrade(upgrade)
	}
}
...
func NewNodeWithContext(
	...
	if err := validateBuildServices(config); err != nil {
		return nil, err
	}
	blockStore, stateDB, err := initDBs(config, dbProvider)
	if err != nil {
	...
	if useCometNetworking {
		...
		reactors := []optionalSwitchReactor{
		...
		sw, err = createOptionalSwitch(config, nodeInfo, nodeKey, reactors, p2pMetrics, p2pLogger, state.LastBlockHeight)
		if err != nil {
			return nil, err
		}
	...
	if node.authenticatedTransportError != nil {
		return nil, node.authenticatedTransportError
	}
...
func (n *Node) OnStart() error {
	...
	var (
		pprofSrv, prometheusSrv auxiliaryServer
		pprofLn                 net.Listener
	...
	if n.config.Instrumentation.IsPrometheusEnabled() {
		var err error
		prometheusSrv, err = n.startPrometheusServer()
		if err != nil {
			return err
		}
	}
