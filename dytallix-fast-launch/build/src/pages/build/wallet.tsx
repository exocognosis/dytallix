...
export function WalletPage() {
  ...
  const blockchainBase = (() => {
    const rawBase = buildApiUrl('/blockchain').replace(/\/+$/, '')
    if (typeof window === 'undefined') {
      return rawBase || '/api/blockchain'
    }

    const fallback = new URL('/api/blockchain', window.location.origin).toString().replace(/\/+$/, '')
    if (!rawBase) {
      return fallback
    }
    ...
    try {
      return new URL(rawBase, window.location.origin).toString().replace(/\/+$/, '')
    } catch {
      return fallback
    }
  })()

  const requestFaucetTokens = async (payload: { address: string, dgt_amount: number, drt_amount: number }) => {
    try {
      return await fetch(buildApiUrl('/faucet/request'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
    } catch (error) {
      console.error('[Wallet] Faucet request failed', error)
      return null
    }
  }
  ...
  const checkBalance = async (walletId: string, address: string) => {
    ...
    try {
      const response = await fetch(`${blockchainBase}/balance/${address}`);
      if (response.ok) {
  ...
  const generateWallet = () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const response = await requestFaucetTokens({
          address: addr,
          dgt_amount: 1000,
          drt_amount: 10000
        })
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${blockchainBase}/balance/${searchQuery}`);
      if (response.ok) {
        const data = await response.json();
        const balances = data.balances || {};
        const getVal = (val: any) => val?.balance || val || 0;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const statusRes = await fetch(`${blockchainBase}/status`);
      ...
      const accountData = await fetch(`${blockchainBase}/account/${senderWallet.address}`).then(r => r.json());
      ...
      // 5. Submit
      const submitRes = await fetch(`${blockchainBase}/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
