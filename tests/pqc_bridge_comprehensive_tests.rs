...
use dytallix_pqc::{BridgePQCManager, BridgeSignature, CrossChainPayload, SignatureAlgorithm};
...
mod pqc_bridge_tests {
    ...
    fn test_valid_bridge_signature_verification() {
        ...
        assert!(
            result.is_ok(),
            "Asset locking should succeed with valid PQC signatures"
        );

        let tx_id = result.unwrap();
        println!(
            "✅ Successfully locked asset with PQC signatures: {}",
            tx_id.0
        );
    }
    ...
    fn test_multi_algorithm_signature_verification() {
        ...
        for (name, algorithm) in algorithms {
            ...
            let result = bridge.lock_asset(
                asset,
                "ethereum",
                "0x742d35Cc6634C0532925a3b8D1EbA4F00b7C8000",
            );
            assert!(
                result.is_ok(),
    ...
    fn test_insufficient_signatures() {
        ...
        assert!(
            result.is_err(),
            "Should reject transaction with insufficient signatures"
        );
    ...
    fn test_wrong_validator_key_scenarios() {
        ...
        let legitimate_keypair = pqc_manager
        ...
        let legitimate_signature = pqc_manager
            .sign_bridge_payload(&payload, "ethereum", "legitimate_validator")
            .unwrap();
        let is_valid = pqc_manager
            .verify_bridge_signature(&legitimate_signature, &payload)
            .unwrap();
...
mod pqc_performance_tests {
    ...
    fn benchmark_pqc_signature_generation() {
        ...
        for (name, algorithm) in algorithms {
            ...
            let keypair = local_manager
                .generate_validator_keypair(&algorithm)
                .unwrap();
            ...
            for _ in 0..5 {
                let _ =
                    local_manager.sign_bridge_payload(&payload, "ethereum", "benchmark_validator");
            }
            ...
            for _ in 0..iterations {
                let signature = local_manager
                    .sign_bridge_payload(&payload, "ethereum", "benchmark_validator")
                    .unwrap();
    ...
    fn benchmark_multi_signature_verification() {
        ...
        assert!(
            ...
            "Multi-signature verification should complete within 5 seconds"
        );
...
mod pqc_security_tests {
    ...
    fn test_payload_tampering_detection() {
        ...
        let signature = pqc_manager
            .sign_bridge_payload(&original_payload, "ethereum", "security_validator")
            .unwrap();

        // Verify original payload works
        assert!(pqc_manager
            .verify_bridge_signature(&signature, &original_payload)
            .unwrap());
        ...
        let tampered_payloads = vec![
            ...
            CrossChainPayload::GenericBridgePayload {
                asset_id: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { asset_id, .. } => asset_id.clone(),
                    _ => unreachable!(),
                },
                amount: 2000000,
                source_chain: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { source_chain, .. } => {
                        source_chain.clone()
                    }
                    _ => unreachable!(),
                },
                dest_chain: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { dest_chain, .. } => {
                        dest_chain.clone()
                    }
                    _ => unreachable!(),
                },
                source_address: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { source_address, .. } => {
                        source_address.clone()
                    }
                    _ => unreachable!(),
                },
                dest_address: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { dest_address, .. } => {
                        dest_address.clone()
                    }
                    _ => unreachable!(),
                },
                metadata: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { metadata, .. } => metadata.clone(),
                    _ => unreachable!(),
                },
            },
            ...
            CrossChainPayload::GenericBridgePayload {
                ...
                amount: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { amount, .. } => *amount,
                    _ => unreachable!(),
                },
                source_chain: match &original_payload {
                    CrossChainPayload::GenericBridgePayload { source_chain, .. } => {
        ...
        for (i, tampered_payload) in tampered_payloads.iter().enumerate() {
            let is_valid = pqc_manager
                .verify_bridge_signature(&signature, tampered_payload)
                .unwrap();
    ...
    fn test_replay_attack_resistance() {
        ...
        let signature1 = pqc_manager
            .sign_bridge_payload(&payload, "ethereum", "replay_validator")
            .unwrap();
        std::thread::sleep(std::time::Duration::from_millis(10)); // Ensure different timestamp
        let signature2 = pqc_manager
            .sign_bridge_payload(&payload, "ethereum", "replay_validator")
            .unwrap();

        // Both signatures should be valid individually
        assert!(pqc_manager
            .verify_bridge_signature(&signature1, &payload)
            .unwrap());
        assert!(pqc_manager
            .verify_bridge_signature(&signature2, &payload)
            .unwrap());
    ...
    fn test_signature_malleability_resistance() {
        ...
        let signature = pqc_manager
            .sign_bridge_payload(&payload, "ethereum", "malleability_validator")
            .unwrap();
        ...
        let is_valid = pqc_manager
            .verify_bridge_signature(&malleated_signature, &payload)
            .unwrap();
...
mod pqc_integration_tests {
    ...
    fn test_emergency_halt_and_resume() {
        ...
        assert!(
            lock_result.is_err(),
            "Operations should be blocked during halt"
        );
