...
async fn test_successful_single_token_request(endpoint: &str) {
    ...
    assert_eq!(
        response.status(),
        200,
        "Expected HTTP 200 for valid request"
    );
    ...
    let dispensed = body["dispensed"]
        .as_array()
        .expect("dispensed should be array");
...
async fn test_successful_dual_token_request(endpoint: &str) {
    ...
    assert_eq!(
        response.status(),
        200,
        "Expected HTTP 200 for valid dual token request"
    );
...
async fn test_invalid_address_error(endpoint: &str) {
    ...
    assert_eq!(
        response.status(),
        400,
        "Expected HTTP 400 for invalid address"
    );
...
async fn test_invalid_token_error(endpoint: &str) {
    ...
    assert_eq!(
        response.status(),
        400,
        "Expected HTTP 400 for invalid token"
    );
...
async fn test_rate_limiting(endpoint: &str) {
    ...
    if second_response.status() == 429 {
        let body: Value = second_response
            .json()
            .await
            .expect("Failed to parse rate limit response");
        validate_error_response_structure(&body);

        // Rate limit error should be either RATE_LIMIT_EXCEEDED or similar
        assert!(
            body["error"].is_string(),
            "Rate limit error should have error field"
        );
        assert!(
            body["message"]
                .as_str()
...
fn validate_success_response_structure(body: &Value) {
    ...
    assert!(
        body["dispensed"].is_array(),
        "dispensed field should be array"
    );
    assert!(
    ...
    if body.get("timestamp").is_some() {
        assert!(
            ...
            "timestamp should be string if present"
        );
    }
}
...
fn validate_token_fields(token: &Value) {
    ...
    assert_eq!(
        tx_hash.len(),
        66,
        "txHash should be 66 characters (0x + 64 hex chars), got: {}",
        tx_hash.len()
    );
    assert!(
