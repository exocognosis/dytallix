...
mod security_tests {
    ...
    fn test_replay_attack_vulnerability() {
        ...
        // Sign the payload
        let signature1 = bridge
            .sign_bridge_payload(&payload, "ethereum", "validator1")
            .unwrap();

        // SECURITY VULNERABILITY: Same signature can be reused (replay attack)
        let signature2 = bridge
            .sign_bridge_payload(&payload, "ethereum", "validator1")
            .unwrap();
        ...
        assert_ne!(
            signature1.signature.data, signature2.signature.data,
    ...
    fn test_memory_safety_key_handling() {
        ...
        assert_ne!(
            ...
            "Key rotation should generate new key"
        );
    ...
    fn test_performance_security_tradeoffs() {
        ...
        for _ in 0..iterations {
            let signature = manager.sign(message).unwrap();
            let _ = manager
                .verify(message, &signature, manager.get_signature_public_key())
                .unwrap();
        }
