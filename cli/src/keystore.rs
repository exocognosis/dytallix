...
fn timeout() -> Duration {
    let secs: u64 = std::env::var("DYL_KEY_TIMEOUT_SECS")
        .ok()
...
fn purge_expired() {
    ...
    w.retain(|k| k.last_used.elapsed() <= to);
}
...
pub fn get_unlocked(name: &str) -> Option<SecretGuard> {
    purge_expired();
    let mut w = UNLOCKED.write().unwrap();
    if let Some(pos) = w.iter().position(|k| k.name == name) {
        let mut uk = w[pos].clone();
        if uk.last_used.elapsed() > timeout() {
            w.remove(pos);
            return None;
        }
        uk.mark_used();
        ...
        return Some(SecretGuard {
            ...
            pk: uk.pk.clone(),
            address: uk.address,
            algorithm: uk.algorithm,
        });
    }
    None
}

pub fn keystore_path(home: &str) -> PathBuf {
    let expanded = shellexpand::tilde(home).to_string();
    PathBuf::from(expanded).join("keystore.json")
}
...
pub fn save(home: &str, ks: &KeystoreFile) -> Result<()> {
    let path = keystore_path(home);
...
fn derive_key(password: &str, salt: &[u8], kp: &KdfParams) -> Result<[u8; 32]> {
    let params =
        Params::new(kp.m_cost, kp.t_cost, kp.p, Some(32)).map_err(|e| anyhow!(e.to_string()))?;
...
fn decrypt_sk(password: &str, enc: &EncBlob) -> Result<Vec<u8>> {
    if enc.alg != "chacha20poly1305" {
        return Err(anyhow!("unsupported alg"));
    }
    if enc.kdf.alg != "argon2id" {
        return Err(anyhow!("unsupported kdf"));
    }
    let salt = B64.decode(&enc.salt)?;
    let nonce_bytes = B64.decode(&enc.nonce)?;
    let kp = &enc.kdf;
    let key = derive_key(password, &salt, kp)?;
    let cipher = XChaCha20Poly1305::new(Key::from_slice(&key));
...
pub fn create_new_with_algorithm(
    home: &str,
    name: &str,
    password: &str,
    algorithm: &str,
    ...
    if ks.keys.iter().any(|k| k.name == name) {
    ...
    save(home, &ks)?;
    info!(
        "event=keystore_create name={} algorithm={}",
        name, algorithm
    );
    Ok(ent)
}

pub fn list(home: &str) -> Result<Vec<KeystoreEntry>> {
    Ok(load_or_init(home)?.keys)
}
...
pub fn change_password(home: &str, name: &str, old: &str, newp: &str) -> Result<()> {
    ...
    let sk = decrypt_sk(old, &ent.enc)?;
    let kp = KdfParams::default();
    ent.enc = encrypt_sk(newp, &sk, kp)?;
    save(home, &ks)?;
    Ok(())
}
...
pub fn unlock(home: &str, name: &str, password: &str) -> Result<UnlockedKey> {
    ...
    if *FAILS.read().unwrap() >= 5 {
        warn!(
            "event=unlock_blocked name={} reason=too_many_failures",
            name
        );
        return Err(anyhow!("too many failures"));
    }
    let ks = load_or_init(home)?;
    let ent = ks
        .keys
        .into_iter()
        .find(|k| k.name == name)
        .ok_or(anyhow!("not found"))?;
    let sk = match decrypt_sk(password, &ent.enc) {
        Ok(v) => v,
        Err(e) => {
            let mut f = FAILS.write().unwrap();
            *f += 1;
            warn!(
                "event=unlock_failure name={} attempt={} error=decrypt",
                name, *f
            );
            return Err(e);
        }
    ...
    let uk = UnlockedKey {
        ...
        sk,
        pk: pk.clone(),
        address: ent.address.clone(),
        last_used: Instant::now(),
        algorithm: ent.algorithm.clone(),
    };
    {
        let mut w = UNLOCKED.write().unwrap();
...
mod tests {
    ...
    fn roundtrip() {
        ...
        let e = create_new(home, "def", "pw").unwrap();
        assert_eq!(e.name, "def");
        let listv = list(home).unwrap();
        assert_eq!(listv.len(), 1);
