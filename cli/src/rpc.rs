...
impl RpcClient {
    ...
    pub async fn get_nonce(&self, address: &str) -> Result<Option<u64>> {
        ...
        if !res.status().is_success() {
            return Err(anyhow!(format!("nonce fetch error {}", res.status())));
        }
        let v: serde_json::Value = res.json().await?;
        if let Some(n) = v["nonce"].as_u64() {
            Ok(Some(n))
        } else {
            Err(anyhow!("malformed nonce response"))
        }
    }
    ...
    pub async fn submit(&self, stx: &SignedTx) -> Result<BroadcastResponse> {
        ...
        for path in ["/submit", "/tx/broadcast"] {
            ...
            let br: BroadcastResponse = resp.json().await?;
            return Ok(br);
        }
