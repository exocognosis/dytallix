...
use crate::tx::{de_nonce, de_u128_string, NonceSpec};
...
pub struct Batch {
    ...
    pub nonce: NonceSpec,
    #[serde(deserialize_with = "de_u128_string")]
    pub fee: u128,
    #[serde(default)]
    pub memo: String,
...
impl Batch {
    pub fn validate(&mut self) -> Result<()> {
        if self.chain_id.trim().is_empty() {
            return Err(anyhow!("empty chain_id"));
        }
        ...
        for m in &mut self.messages {
            match m {
                BatchMsg::Send { denom, amount, .. } => {
                    if *amount == 0 {
                        return Err(anyhow!("amount must be > 0"));
                    }
                    *denom = denom.to_ascii_uppercase();
                    if denom != "DGT" && denom != "DRT" {
                        return Err(anyhow!("invalid denom: {}; valid: DGT, DRT", denom));
                    }
                }
            }
        }
