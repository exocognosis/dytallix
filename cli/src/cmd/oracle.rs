...
use crate::{output::OutputFormat, rpc::RpcClient};
...
pub async fn run(rpc_url: &str, output_format: OutputFormat, cmd: OracleCmd) -> Result<()> {
    ...
    match cmd.action {
        ...
        OracleAction::SubmitBatch { file } => submit_batch(&client, &file, output_format).await,

        OracleAction::Query { tx_hash } => query_risk(&client, &tx_hash, output_format).await,

        OracleAction::Stats => get_stats(&client, output_format).await,
    }
}
...
async fn submit_single(
    ...
    if !tx_hash.starts_with("0x") {
        return Err(anyhow::anyhow!(
            "tx_hash must be in hex format starting with 0x"
        ));
    }
...
async fn submit_batch(
    ...
    for (idx, record) in records.iter().enumerate() {
        ...
        let risk_score = record
            .get("risk_score")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| anyhow::anyhow!("Record {}: missing or invalid risk_score", idx))?
            as f32;

        // Validate values
        if !tx_hash.starts_with("0x") {
            return Err(anyhow::anyhow!(
                "Record {}: tx_hash must start with 0x",
                idx
            ));
        }
        ...
        if !(0.0..=1.0).contains(&risk_score) {
            return Err(anyhow::anyhow!(
                "Record {}: risk_score must be between 0.0 and 1.0",
                idx
            ));
        }
        ...
        if let Some(confidence) = record.get("confidence").and_then(|v| v.as_f64()) {
            if !(0.0..=1.0).contains(&confidence) {
                return Err(anyhow::anyhow!(
                    "Record {}: confidence must be between 0.0 and 1.0",
                    idx
                ));
            }
        }
...
async fn query_risk(client: &RpcClient, tx_hash: &str, output_format: OutputFormat) -> Result<()> {
    if !tx_hash.starts_with("0x") {
        return Err(anyhow::anyhow!(
            "tx_hash must be in hex format starting with 0x"
        ));
...
async fn get_stats(client: &RpcClient, output_format: OutputFormat) -> Result<()> {
    ...
    if output_format.is_json() {
        ...
        if let Some(signature_enabled) = response
            .get("signature_verification_enabled")
        ...
        if let Some(validation_rules) = response.get("validation_rules") {
            ...
            if let Some(risk_range) = validation_rules
