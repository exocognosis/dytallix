...
pub async fn run(rpc: &str, fmt: OutputFormat, cmd: QueryCmd) -> Result<()> {
    ...
    match cmd.what {
                        ...
                        for (denom, info) in balance_map {
                            if let Some(formatted) = info.get("formatted") {
                                ...
                                println!(
                                    "  {}: {}",
                                    denom,
                                    info.get("balance").unwrap_or(&serde_json::Value::Null)
                                );
                            }
                        }
                    }
        ...
        QueryWhat::Tx { hash } => {
            ...
            let res = reqwest::get(url).await?;
            if !res.status().is_success() {
                return Err(anyhow!("tx query failed"));
            }
            let v: serde_json::Value = res.json().await?;
            if fmt.is_json() {
                print_json(&v)?;
        ...
        QueryWhat::Block { id } => {
            ...
            let res = reqwest::get(url).await?;
            if !res.status().is_success() {
                return Err(anyhow!("block query failed"));
            }
            let v: serde_json::Value = res.json().await?;
            if fmt.is_json() {
                print_json(&v)?;
                ...
                Ok(res) => {
                    if !res.status().is_success() {
                        ...
                        match reqwest::get(&alt_url).await {
                            ...
                            Err(_) => {
                                return Err(anyhow!(
                                    "emission query failed - endpoints not available"
                                ))
                            }
                        }
                    } else {
        ...
        QueryWhat::Stats => {
            let url = format!("{}/stats", client.base);
            let res = reqwest::get(url).await?;
            let v: serde_json::Value = res.json().await?;
            if fmt.is_json() {
                print_json(&v)?;
