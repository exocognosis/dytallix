...
fn flags_override_config() {
    ...
    cmd.arg("--gov.quorum").arg("0.25").arg("governance").arg("print-effective");
    cmd.assert().success().stdout(contains("quorum=0.25 source=flag"));
}
...
fn config_used_when_no_flags() {
    ...
    cmd.assert().success().stdout(contains("quorum=0.12 source=config"));
}
...
fn invalid_relationship_errors() {
    ...
    cmd.assert().failure().stderr(contains("quorum 0.6 cannot exceed threshold 0.5"));
}
...
fn deposit_parsing_denied_wrong_denom() {
    ...
    let mut cmd = Command::cargo_bin("dcli").unwrap();
    cmd.arg("governance").arg("validate");
    cmd.assert().failure().stderr(contains("unexpected denom foo"));
}
...
fn init_idempotent_patch() {
    ...
    first.arg("init").arg("--genesis-out").arg("genesis-test.json");
    ...
    second.arg("init").arg("--genesis-out").arg("genesis-test.json");
...
fn init_force_overwrites() {
    write_governance("quorum='0.33'\nthreshold='0.50'\nveto='0.334'\ndeposit_min='1000udgt'\nvoting_period='100'\n");
    let mut first = Command::cargo_bin("dcli").unwrap();
    first.arg("init").arg("--genesis-out").arg("genesis-force.json");
    first.assert().success();
    fs::write("genesis-force.json", "{\"other\":123}").unwrap();
    let mut force = Command::cargo_bin("dcli").unwrap();
    force.arg("init").arg("--genesis-out").arg("genesis-force.json").arg("--force");
