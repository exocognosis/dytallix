...
mod tests {
    ...
    fn test_address_derivation_format() {
        ...
        assert!(address.starts_with("dytallix"), "Address must start with 'dytallix'");
        assert!(address.len() > 8, "Address must be longer than just the prefix");
        
        // Test address contains only valid characters (hex after prefix)
        let hex_part = &address["dytallix".len()..];
        assert_eq!(hex_part.len(), 40, "Address hash should be 20 bytes (40 hex chars)");
        assert!(hex_part.chars().all(|c| c.is_ascii_hexdigit()), "Address hash must be valid hex");
    }
    ...
    fn test_argon2_params_floor() {
        ...
        assert!(params.memory_cost >= 64 * 1024, "Memory cost must be at least 64 MiB");
    ...
    fn test_address_derivation_vectors() {
        ...
        let address = derive_address(&test_pubkey).unwrap();
        
        assert!(address.starts_with("dytallix"));
        
        // Test same input produces same output
        let address2 = derive_address(&test_pubkey).unwrap();
    ...
    fn test_signature_size() {
        ...
        assert!(signature.data.len() > 4000, "Dilithium5 signature too small");
        assert!(signature.data.len() < 5000, "Dilithium5 signature too large");
    }
    ...
    fn test_key_generation_performance() {
        ...
        assert!(duration.as_secs() < 10, "Key generation too slow: {:?}", duration);
