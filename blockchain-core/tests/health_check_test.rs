...
use dytallix_node::consensus::{AIOracleClient, AIHealthCheckResponse, AIServiceStatus};
...
async fn test_health_check_with_timeout() {
    ...
    let health_response = client.health_check_with_timeout(Duration::from_millis(1)).await;
    assert!(health_response.is_ok());
    
    let health = health_response.unwrap();
    // Should be degraded due to timeout
    assert!(matches!(health.status, AIServiceStatus::Degraded | AIServiceStatus::Unhealthy));
...
async fn test_health_check_with_invalid_service() {
    ...
    assert!(matches!(health.status, AIServiceStatus::Unhealthy | AIServiceStatus::Unknown));
    assert!(health.details.is_some());
    assert!(health.response_time_ms > 0);
...
async fn test_health_check_response_parsing() {
    ...
    assert!(health.timestamp > 0);
    assert!(health.response_time_ms >= 0);
    assert!(matches!(health.status, AIServiceStatus::Healthy | AIServiceStatus::Degraded | AIServiceStatus::Unhealthy | AIServiceStatus::Unknown));
...
fn test_health_check_response_creation() {
    use dytallix_node::consensus::{AIHealthCheckResponse, AIServiceStatus, AIServiceLoad};
