...
use dytallix_node::types::{DeployTransaction, CallTransaction, Transaction, PQCTransactionSignature, Block};
...
async fn test_basic_contract_integration() {
    ...
    assert!(validation_result.is_ok(), "Deploy block validation should succeed");
...
async fn test_contract_call_integration() {
    ...
    let validation_result = consensus_engine.validate_block(&block).await;
    assert!(validation_result.is_ok(), "Call block validation should succeed");
