...
use dytallix_blockchain_core::types::{
    Block, BlockHeader, Transaction, TransferTransaction, AIRequestTransaction,
    PQCTransactionSignature, PQCBlockSignature, AIResponsePayload
};
...
async fn test_ai_response_verification() -> Result<()> {
    ...
    let result = ai_integration.verify_ai_response(&test_data.signed_response).await;
...
async fn test_oracle_registry_operations() -> Result<()> {
    ...
    verifier.update_oracle_reputation("test-oracle-registry", 0.9).await?;
