...
pub trait OracleState {
    ...
    fn set_oracle_risk(&mut self, tx_hash: &str, record: &AiRiskRecord) -> Result<(), Box<dyn std::error::Error>>;
...
mod tests {
    ...
    fn test_deterministic_score_preservation() {
        ...
        apply_oracle_risk(&mut state, tx_hash, original_score, model_id, timestamp, source).unwrap();
