...
mod tests {
    ...
    fn test_ai_service_load_calculations() {
        ...
        // Adapted expectations for Option-based fields
        assert_eq!(load.cpu_usage.unwrap(), 60.0);
        assert_eq!(load.memory_usage.unwrap(), 45.0);

        // (Legacy load percentage logic removed; placeholder assertion)
        assert!(load.avg_response_time_ms.unwrap() <= 500.0);

        // Simulate update (no helper methods in new struct)
    ...
    fn test_signed_ai_oracle_response() {
        ...
        let signed_response = SignedAIOracleResponse::new(
            ...
            signature,
            12345,                               // nonce
            (chrono::Utc::now().timestamp() + 60) as u64, // expires_at in 1 minute
            oracle_identity,
        );
