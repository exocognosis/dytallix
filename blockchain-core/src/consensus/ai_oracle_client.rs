...
impl AIOracleClient {
    ...
    pub async fn post<P: Serialize + ?Sized, R: DeserializeOwned>(
        ...
        while attempts < self.config.max_retries {
            ...
            if response.status().is_success() {
                ...
                return Err(anyhow!("HTTP error {}: {}", response.status(), response.text().await?));
            }
        }
    ...
    pub async fn batch_request_analyses(
        ...
        let responses: Vec<SignedAIOracleResponse> = self.post("batch_analyze", &batch_payload).await?;
        Ok(responses)
    }
    ...
    pub async fn is_service_available(&self, service_type: AIServiceType) -> Result<bool> {
        ...
        Ok(services.iter().any(|s| s.service_type == service_type && s.availability_score > 0.5))
    }

    /// Get the best available service for a specific type
    pub async fn get_best_service(&self, service_type: AIServiceType) -> Result<Option<AIServiceInfo>> {
        let services = self.discover_services().await?;
        let best_service = services
            .into_iter()
            .filter(|s| s.service_type == service_type)
            .max_by(|a, b| a.availability_score.partial_cmp(&b.availability_score).unwrap_or(std::cmp::Ordering::Equal));
