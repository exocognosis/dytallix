...
use wasmtime::Func;
use super::{engine::WasmEngine, contract_registry::ContractStore, gas_meter::finalize_gas, types::ContractInstance};
...
impl<'a, S: ContractStore> WasmExecutor<'a, S> {
    pub fn deploy(&mut self, creator: [u8;32], code: &[u8], height: u64, gas_limit: u64) -> Result<(ContractInstance, u64)> {
        ...
        if let Ok(init) = instance.get_func(&mut store_ctx, "init") { 
    ...
    pub fn execute(&mut self, address: [u8;32], method: &str, gas_limit: u64) -> Result<(i64, u64)> {
        ...
        let func: Func = instance.get_func(&mut store_ctx, method).ok_or_else(|| anyhow!("method not found"))?;
