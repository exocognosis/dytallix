...
pub trait ContractStore {
    ...
    fn create_instance(&mut self, code_hash: CodeHash, creator: Address, height: u64) -> Result<ContractInstance>;
...
impl ContractStore for InMemoryContractStore {
    ...
    fn get_code(&self, hash: &CodeHash) -> Result<Vec<u8>> { 
        self.code.get(&hash.to_vec()).cloned().ok_or_else(|| anyhow!("code not found")) 
    }
    ...
    fn get_instance(&self, addr: &Address) -> Result<ContractInstance> { 
        self.instances.get(&addr.to_vec()).cloned().ok_or_else(|| anyhow!("instance not found")) 
    }
    
    fn update_instance(&mut self, instance: &ContractInstance) -> Result<()> { 
        self.instances.insert(instance.address.to_vec(), instance.clone()); 
        Ok(()) 
    }
}